...
node_keys = {
    name: {
        "publicKey": os.getenv(f"PY_TESTNET_GNOSIS_{name.replace(' ', '').upper()}_PUBLIC_KEY"),
        "privateKey": os.getenv(f"PY_TESTNET_GNOSIS_{name.replace(' ', '').upper()}_PRIVATE_KEY"),
    }
...
def run_test_for_node(node, index):
    ...
    for i in range(10):
        ...
        try:
            ...
            ual = "did:dkg:gnosis:10200/0x592aae7abeed0ecf399c2628b3d18f769c544383/55319"
